...
public abstract class Configuracao {
	...
	public static void carregarConfiguracao() throws Exception {
		...
		try {
			...
			if (arquivoXML == null) {
				throw new Exception("ARQUIVO XML DE CONFIGURACOES NÃO FOI ENCONTRADO.");
			}
	...
	protected static File consultarXMLConfiguracao() throws Exception {
		...
		if (arquivo != null) {
			...
			LogUtil.Info("ARQUIVO DE CONFIGURACAO NÃO ENCONTRADO, OBTENDO ARQUIVO PADRAO.");
	...
	public static void removerArquivoConfig() {
		LogUtil.Info("REMOVENDO ARQUIVO XML DE CONFIGURAÇÃO ...");
		if (arquivoXML.delete()) {
			LogUtil.Info("ARQUIVO XML DE CONFIGURAÇÃO FOI REMOVIDO COM SUCESSO.");
		} else {
			LogUtil.Warn("ARQUIVO XML DE CONFIGURAÇÃO NÃO PODE SER REMOVIDO.");
		}
	}
